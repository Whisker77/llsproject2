...
class RAGQueryService:
    def __init__(
            ...
            milvus_db:str = settings.MILVUS_DEFAULT_DB,
    ...
    def _init_vector_store(self, connection_args: Dict, collection_name: str) -> Milvus:
        """初始化Milvus向量存储（修复collection not exist问题）"""
        try:
            # 1. 补充关键连接参数（这些只给 pymilvus 用）
            connection_args = {
                **connection_args,
                "protocol": "grpc",
                "enable_async": False,
            ...
            logger.info(
                f"[RAG] CONNECT TO host={connection_args['host']} port={connection_args['port']} db={self.milvus_db} alias={alias}")
            ...
            connections.connect(
                ...
                db_name=self.milvus_db,  # ✅ 建议改成 self.milvus_db
                protocol=connection_args["protocol"],
                enable_async=connection_args["enable_async"]
            )

            # ✅ 4. 强制切库（关键）
            db.using_database(db_name=self.milvus_db, using=alias)
            logger.info(f"[RAG] db={self.milvus_db}, collections={utility.list_collections(using=alias)}")
            ...
            if utility.has_collection(collection_name, using=alias):
                ...
                raise RAGException(500, f"集合 '{collection_name}' 不存在（db={self.milvus_db}, alias={alias}）")

            # ✅ 7. 用干净参数给 langchain_milvus
            lc_connection_args = {
                "host": connection_args["host"],
                "port": connection_args["port"],
                "token": connection_args["token"],
            ...
            vector_store = Milvus(
                ...
                connection_args=lc_connection_args,
                auto_id=True,
                drop_old=False
            )

            # 8. 检查集合数据（仅当require_data为True时）
            if self.require_data:
                coll = Collection(name=collection_name, using=alias)
                coll.load()
                logger.info(f"集合 '{collection_name}' loaded, entities={coll.num_entities}")
...
#         )
    ...
    def query_score(self, user_question: str, file_id: Optional[str] = None,collection_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.qa_chain.invoke({"query": user_question.strip()}) #大模型的所有返回

            # 4. 解析并验证LLM输出
            json_match = re.search(r'\{[\s\S]*?\}', result["result"].strip()) #json_match是一个对象
            if not json_match:
                raise RAGQueryException(500, f"LLM未输出有效JSON：{result['result'][:100]}")

            try:
                score_data = json.loads(json_match.group()) #大模型回答，转成字典形式
